<?php
  ...
  $id = $_GET['id'];
  $qu = "SELECT * FROM MASUK WHERE ID = '$id'";
                                ...
                                <div class="col-md-6">
                                    ...
                                    <form role="form" action="<?PHP echo $_SERVER['SCRIPT_NAME'] ?>" method="POST">
                                        ...
                                        <div class="form-group input-group-sm">
                                            ...
                                            <input class="form-control" name="NO" maxlength="90" size="60" value="<?php echo $query_data[2]; ?>" />
                                        </div>
                                        <div class="form-group input-group-sm">
                                            <label>Alamat</label>
                                            <input class="form-control" name="ALAMAT" maxlength="90" size="60" value="<?php echo $query_data[3]; ?>" />
                                        </div>
                                        <div class="form-group input-group-sm">
                                            <label>Klasifikasi Pindah</label>
                                            <input class="form-control" name="KLASIFIKASI" maxlength="90" size="60" value="<?php echo $query_data[4]; ?>" />
                                        </div>
                                        <div class="form-group input-group-sm">
                                            <label>Alasan Pindah</label>
                                            <input class="form-control" name="ALASAN" maxlength="90" size="60" value="<?php echo $query_data[5]; ?>" />
                                        </div>
                                        <div align="right">
...
<?php 
  ...
  if($edit) {
    if($id == "" || $tanggal == "" || $no == "" || $alamat == "" || $klasifikasi == "" || $alasan == "" ){
      ?>
      <script type="text/javascript">alert("inputan tidak boleh kosong");
      </script>
      <?php
    }else {
        mysqli_query($connection, "update MASUK set TANGGAL = '$tanggal', NO = '$no', ALAMAT = '$alamat', KLASIFIKASI = '$klasifikasi', ALASAN = '$alasan' where ID = '$id'") or die (mysql_error());
        ?>
