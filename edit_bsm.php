<?php
  ...
  $id = $_GET['id'];
  $qu = "SELECT * FROM BSM WHERE ID = '$id'";
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <form role="form" action="<?PHP echo $_SERVER['SCRIPT_NAME'] ?>" method="POST">
                                        ...
                                        <div class="form-group input-group-sm">
                                            <label>Raskin</label>
                                            <input class="form-control" name="PENDIDIKAN" maxlength="90" size="60" value="<?php echo $query_data[4]; ?>" />
                                        </div>
                                        <div class="form-group input-group-sm">
                                            <label>BPJS</label>
                                            <input class="form-control" name="MENERIMA" maxlength="90" size="60" value="<?php echo $query_data[5]; ?>" />
                                        </div>
                                        <div class="form-group input-group-sm">
                                            <label>Status Ekonomi</label>
                                            <input class="form-control" name="STATUS" maxlength="90" size="60" value="<?php echo $query_data[6]; ?>" />
                                        </div>
                                        <div align="right">
...
<?php 
  ...
  if($edit) {
    if($id == "" || $kartu == "" || $nik == "" || $nama == "" || $pendidikan == "" || $menerima == "" || $status == "" ){
      ?>
      <script type="text/javascript">alert("inputan tidak boleh kosong");
      </script>
      <?php
    }else {
        mysqli_query($connection, "update BSM set KARTU = '$kartu', NIK = '$nik', NAMA = '$nama', PENDIDIKAN = '$pendidikan', MENERIMA = '$menerima', STATUS = '$status' where ID = '$id'") or die (mysql_error());
        ?>
        <script type="text/javascript">
        alert("Data berhasil diubah");
        window.location.href="bsm.php";
        </script>
        <?php
      }
    }
